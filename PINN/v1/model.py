...
class Model(nn.Module):
    def __init__(
        ...
        # 3 -> hidden -> ... -> hidden -> 2
        modules = get_slp_layer(in_dim, hidden_dim, use_bias, bn_momentum, dropout)
        for _ in range(depth - 2):
            modules.extend(
                get_slp_layer(hidden_dim, hidden_dim, use_bias, bn_momentum, dropout)
            )
        modules.extend(
            get_slp_layer(
                hidden_dim, out_dim, use_bias, bn_momentum, dropout, last=True
            )
        )
