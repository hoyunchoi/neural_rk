...
def preprocess(data: pd.Series) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    dx, dy = edge2dxdy(data.edge_attr, Nx, Ny)  # [Nx, ], [Ny, ]
    x = np.insert(np.cumsum(dx), 0, 0.0)  # [Nx+1, ]
    y = np.insert(np.cumsum(dy), 0, 0.0)  # [Ny+1, ]
    ...
    field = np.stack(
        [to_periodic_field(node2grid(traj, Nx, Ny)) for traj in data.trajectories],
        axis=0,
    )
