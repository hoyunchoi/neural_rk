...
def burgers_graph(
    ...
    """
    is_not_boundary: [N, 1], 1 if the node is not boundary
    ...
    # Return: [N, 1, 2] -> [N, 2], only remain non-boundary values
    delta = torch.sum(-field.unsqueeze(1) * jacobian + nu * hessians, dim=2)
    return delta * is_not_boundary
...
def rk1(
    ...
    """
    delta_field = burgers_graph(is_not_boundary, edge_list, edge_attr, nu, field)
...
def rk2(
    ...
    Return: [N, 2] next field
    """
    delta_field1 = burgers_graph(is_not_boundary, edge_list, edge_attr, nu, field)

    temp_field = field + dt * delta_field1
    delta_field2 = burgers_graph(is_not_boundary, edge_list, edge_attr, nu, temp_field)
...
def rk4(
    ...
    delta_field3 = burgers_graph(is_not_boundary, edge_list, edge_attr, nu, temp_field)

    temp_field = field + dt * delta_field3
    delta_field4 = burgers_graph(is_not_boundary, edge_list, edge_attr, nu, temp_field)
