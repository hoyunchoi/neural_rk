...
from burgers_dirichlet import trajectory
from burgers_dirichlet.simulation import argument, solve_grid
from graph import get_square_grid
from graph.utils import get_edge_list
from neural_rk.grid_graph_dirichlet import dxdy2edge, dxdy2pos, grid2node
from neural_rk.path import DATA_DIR
from neural_rk.simulation_data import SimulationData, to_df

Lx, Ly, max_time = 1.0, 1.0, 1.0  # x, y length of the domain
...
def main() -> None:
    ...
    get_initial_condition: Callable[[argument.arr], argument.arr] = lambda x: x
    ...
    while len(data) < args.num_samples:
        ...
        # Get initial field
        initial_field = get_initial_condition(position)  # [Ny, Nx, 2]
    ...
    df.to_pickle(DATA_DIR / f"burgers_dirichlet_{args.name}.pkl")
