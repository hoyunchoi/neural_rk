...
# from .dataset import EfficientDataset as Dataset
...
def get_loss_fn(name: str) -> Callable[[torch.Tensor, torch.Tensor], torch.Tensor]:
    def mape_loss(
        prediction: torch.Tensor, target: torch.Tensor, eps: float = 1e-4
    ) -> torch.Tensor:
        mask = target < eps
        abs_err = (prediction - target).abs()

        ape = (abs_err[mask] / target[mask]).sum() + abs_err[~mask].sum()
        return ape / prediction.numel()

    def mse_heat(prediction: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        return F.mse_loss(prediction, target) + 1e-3 * prediction.mean()
...
def run(
    ...
    grad_scaler_state_dict: dict[str, torch.Tensor] | None = None,
    save: bool = True,
) -> None:
    ...
    if is_ddp:
        model = cast(
            ...
            DDP(
                model,
                static_graph=hp.window == 1,  # type:ignore
                gradient_as_bucket_view=True,  # type:ignore
            ),
            # DDP(model, find_unused_parameters=True),  # debug mode
        )

    # Optimizer, scheduler, gradient scaler configuration
    ...
    if is_prime:
        ...
        grad_scaler_state_dict = copy.deepcopy(grad_scaler.state_dict())
    else:
    ...
    validate_rollout = functools.partial(
        trainer.validate_rollout, is_diverging=IsDiverging()
    )
    train = trainer.train if hp.window == 1 else trainer.train_rollout
    validate = trainer.validate if hp.window == 1 else validate_rollout
    ...
    for epoch in epoch_range:
        ...
        val_loss, val_mae = validate(model, val_loader, loss_fn, hp.amp, device)

        rollout_dataset.sampler.set_epoch(epoch)
        rollout_loss, rollout_mae = validate_rollout(
            model, rollout_loader, loss_fn, hp.amp, device
        )
    ...
    torch.save(checkpoint, result_dir / "best.pth")
    ...
    mae_df = pd.DataFrame(
        np.stack((train_mae, val_mae, rollout_mae), axis=-1),
        columns=["train_mae", "val_mae", "rollout_mae"],
    )
    mae_df.to_csv(result_dir / "mae.txt", sep="\t", index=False)
