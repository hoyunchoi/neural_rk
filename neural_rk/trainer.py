...
def train(
    ...
    for batch_data in data_loader:
        ...
        with autocast(device.type, dtype=amp_dtype(device), enabled=use_amp):
            delta_x = model(
                x=batch_data.x + 2.0 * noise * (torch.rand_like(batch_data.x) - 0.5),
                # x=batch_data.x + noise * torch.randn_like(batch_data.x),
            ...
            loss = loss_fn(delta_x, batch_data.y)  # (BN, state_feature)
            with torch.no_grad():
...
def train_rollout(
    ...
    for batch_data in data_loader:
        ...
        # Tensor to store prediction of model
        pred_delta_x = []

        # Input with random noise
        x = batch_data.x + 2.0 * noise * (torch.rand_like(batch_data.x) - 0.5)
        for dt in batch_data.dt:  # (W, BN, 1)
            with autocast(device.type, dtype=amp_dtype(device), enabled=use_amp):
                delta_x = model(
                ...
                pred_delta_x.append(delta_x)
            x += delta_x * dt

        pred_delta_x = torch.stack(pred_delta_x)
        loss = loss_fn(pred_delta_x, batch_data.y)
        cast(torch.Tensor, grad_scaler.scale(loss)).backward()

        with torch.no_grad():
            mae = F.l1_loss(pred_delta_x, batch_data.y)
            tot_loss += loss
            tot_mae += mae
...
def validate(
    ...
    for batch_data in data_loader:
        ...
        with autocast(device.type, dtype=amp_dtype(device), enabled=use_amp):
            ...
            tot_loss += loss_fn(delta_x, batch_data.y)
...
def validate_rollout(
    ...
    for batch_data in data_loader:
        ...
        with autocast(device.type, dtype=amp_dtype(device), enabled=use_amp):
            ...
            tot_loss += loss_fn(pred_delta_x, batch_data.y)
