...
def get_args(options: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--const_param",
        action="store_true",
        help="If this flag is on, params(dissipation rate) is constant over samples",
    )
    parser.add_argument(
        "--const_ic",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="If given, seed the random engine for graph generation",
    )

    # Parse the arguments and return
    args = parser.parse_args() if options is None else parser.parse_args(options)
    if not args.const_graph:
        # If graph is not constant, then parameters and initial condition cannot be constant
        args.const_param = False
        args.const_ic = False
    return args
